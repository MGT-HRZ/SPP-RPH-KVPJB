<?php
    ...
    function back_to_login($num_dir) {
        ...
        if ($base_url === 'http://localhost') {
            header("Location: {$base_url}/penyemak/index.php?not-allowed");
        } elseif ($base_url === 'http://localhost:3000') {
            header("Location: {$base_url}/penyemak/index.php?not-allowed");
        } elseif ($base_url === 'http://localhost') {
            header("Location: {$base_url}/SPP-RPH-KVPJB/penyemak/index.php?not-allowed");
        } elseif ($base_url === 'http://localhost:3000') {
            header("Location: {$base_url}/SPP-RPH-KVPJB/penyemak/index.php?not-allowed");
            ...
            header("Location: /penyemak/index.php");
        }
